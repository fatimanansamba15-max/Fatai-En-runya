...
def load_translation_engine():
    model_name = "facebook/nllb-Distilled-600M"
    
    # CRITICAL FIX: Use explicit NllbTokenizer to properly handle language tokens
...
def translate_via_neural_net(text, direction_mode):
    try:
        ...
        # CRITICAL FIX 1: Set the source language attribute explicitly onto the tokenizer
        tokenizer.src_lang = src_lang
        
        # Tokenize sequence into tensors and push to active device matrix
        inputs = tokenizer(text, return_tensors="pt").to(device)

        # CRITICAL FIX 2: Map the exact vocabulary index integer target directly from lang_code_to_id
        ...
        translated_tokens = translation_model.generate(
            **inputs,
            ...
            max_length=256,
            num_beams=1,      # Greedy decoding for latency reduction
            do_sample=False   # No sampling overhead variations
        )

        # CRITICAL FIX 3: Safely decode out index 0 to return a clean text string
